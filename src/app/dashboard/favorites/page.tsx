...
const FavoritesPage = async () => {
	const [t, tQrs] = await Promise.all([getTranslations('favorites'), getTranslations('qrs')])
				...
				<div>
					<div className="flex items-center gap-2">
						<span className="bg-warning/10 p-2 rounded-xl">
							<HugeiconsIcon icon={StarIcon} size={20} className="text-warning" />
						</span>
			...
			{qrs.length === 0 ? (
				<div className="flex flex-col items-center justify-center h-64 bg-content1 border border-divider rounded-2xl gap-4">
					<div className="p-4 bg-warning/10 rounded-2xl">
						<HugeiconsIcon icon={StarIcon} size={32} className="text-warning/60" />
					</div>
