...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /login (use exact path as instructed).
        await page.goto("http://localhost:3000/login")
        
        # -> Type the login email into the email field (index 292) and password into the password field (index 293), then click the 'Iniciar sesión' submit button (index 295).
        ...
        metric_text = await frame.locator("xpath=//*[contains(., 'Active QR codes')]").nth(0).text_content()
        assert any(ch.isdigit() for ch in metric_text), "Expected 'Active QR codes' metric to contain a numeric value"
