...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /reset-password (use explicit navigate action to http://localhost:3000/reset-password).
        await page.goto("http://localhost:3000/reset-password")
        
        # -> Type the test email into the email input (index 375) and click the 'Enviar enlace' button (index 376).
        ...
        frame = context.pages[-1]
        assert 'Reset' in await frame.evaluate("() => document.title")
        assert await frame.locator("xpath=//*[contains(., 'email')]").nth(0).is_visible(), "Expected 'email' to be visible"
        assert await frame.locator("xpath=//*[contains(., 'sent')]").nth(0).is_visible(), "Expected 'sent' to be visible"
